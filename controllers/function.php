...
function isValidHeader($jwt, $key)
{
    try {
        ...
        //로그인 함수 직접 구현 요함
        return isValidUser($data->userIdx);
    } catch (\Exception $e) {
        return false;
    }
}
...
function phoneCheck($_str)
{
    if (preg_match("/^(010|011|016|017|018|019)-[^0][0-9]{3,4}-[0-9]{4}/", $_str)) {
        return true;
    }
    else {
        return false;
...
function birthCheck($_str)
{
    if (preg_match("/^(19[0-9][0-9]|20\d{2})-(0[0-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $_str)) {
        return true;
    }
    else {
        return false;
...
function isAvailableAge($birthDay)
{
